...
(function() {
   ...
   *  but uses a different technique to apply rules which avoids the requirement for special syntax
  ...
  */
  ...
  domready(function(){
      ...
      for(var i =0;i<document.styleSheets.length;i++){
        var cssrules;
        if (document.styleSheets[i].cssRules) { cssrules=document.styleSheets[i].cssRules; }
        else if (document.styleSheets[i].rules) { cssrules = document.styleSheets[i].rules; }

          for(var j = 0;j<cssrules.length;j++){
              var rule = cssrules[j], els = getEl(rule.selectorText.replace(/:+\w+/gi,''));
  ...
  function inject(prop, els, rule) {
    ...
    for(var i = 0;i<els.length;i++){
      var elem = els[i];
      var pseudoel = getEl( '.pseudo-after', els[i]);
      if(!pseudoel.length){ 
      ...
      for(var cssprop in style){
        if(style[cssprop]){
          ...
          try{ pseudoel.style[cssprop] = style[cssprop]; } catch(e){ }
        }
      }
    }
  }
  ...
  /* Helper Functions */
     ...
     * logic shamelessly stripped from https://github.com/jquery/
     * Copyright (c) 2012 Alan Clarke
     * Licensed under the GPL license.
     */
